...
static led_panels_buffer *sending_buffer;
...
void led_panels_init(led_panels_io io)
{
  module_io = io;
}

void led_panels_deinit()
{
  module_io = (led_panels_io) { 0 };
}
...
led_panels_status led_panels_send(led_panels_buffer *buffer)
{
  ...
  led_panels_status status = module_io.send(
    buffer->pwm_data,
    96 * sizeof(uint8_t)
  );
  ...
  {
    sending_buffer = buffer;
    sending_buffer->is_locking = true;
  }
...
void led_panels_send_complete(void)
{
  if (sending_buffer == NULL)
    return;

  if (sending_buffer->transmit_index > sending_buffer->pixel_data_size)
  {
    module_io.stop_sending();
    sending_buffer->is_locking = false;
    sending_buffer->transmit_index = 0;
    memset(
      sending_buffer->pwm_data,
      LED_PANELS_0_VALUE,
      96 * sizeof(uint8_t)
    );
    sending_buffer = NULL;
  ...
  {
    memset(
      sending_buffer->pwm_data + 48,
      LED_PANELS_RESET_VALUE,
      48 * sizeof(uint8_t)
    );
    sending_buffer->transmit_index += 6;
    return;
  }
  ...
  set_pixels_to_pwm_data(
    sending_buffer->pwm_data + 48,
    sending_buffer->pixel_data,
    &sending_buffer->transmit_index
  );
}
